...
README.md (Doxygen's main page, via `USE_MDFILE_AS_MAINPAGE`) embeds four plain Markdown images: three from `img/` (`doxygen-preview.png`, `coverlet-preview.png`, `Jira-Rollup-Report.png`) and one from `winner/` (`winner.jpg`, the hackathon award certificate — `winner` is also listed in `IMAGE_PATH` for this reason). Doxygen has no config option that copies plain-Markdown-referenced images into the output directory *preserving their subfolder*: `IMAGE_PATH` only affects images referenced via its own `\image` command, and `HTML_EXTRA_FILES` always flattens whatever it's given into the output root, not `img/`/`winner/`. So each source folder is copied into the matching `doxygen-docs/html/<folder>/` by hand (`cp img/*.png doxygen-docs/html/img/`, `cp winner/winner.jpg doxygen-docs/html/winner/`) rather than through any Doxyfile setting — those manual copies are what the generated `<img src="img/...">`/`<img src="winner/...">` tags actually resolve against.

Separately (confirmed by testing, not assumed): Doxygen **also** auto-copies *every* locally-referenced Markdown image flat into the output root on *every single regenerate* — all four, not just whichever one you most recently touched. This isn't a one-time artifact; it recurs on each `doxygen Doxyfile` run regardless of `IMAGE_PATH`/`HTML_EXTRA_FILES`, and it's an easy step to half-do (only cleaning up the image you were focused on and leaving the other three from before) — that happened once already. Those flat copies (`doxygen-docs/html/doxygen-preview.png`, `coverlet-preview.png`, `Jira-Rollup-Report.png`, `winner.jpg`, sitting directly in `doxygen-docs/html/`) are unused — nothing references them, since the actual `<img src>` in the generated pages points at the `img/`/`winner/` subfolders — so **after every regenerate, delete all four flat copies from the output root**, not just the one for whatever changed; they're harmless clutter, not a broken build, but let git status catch stragglers before committing. If a new image is ever added to `img/` or `winner/` for the README, both steps apply for that new file too: copy it into the matching `doxygen-docs/html/<folder>/` by hand, then delete Doxygen's own flat auto-copy of it from the output root.
