...
README.md (Doxygen's main page, via `USE_MDFILE_AS_MAINPAGE`) embeds three plain Markdown images from `img/` (`doxygen-preview.png`, `coverlet-preview.png`, `Jira-Rollup-Report.png`). Doxygen has no config option that copies plain-Markdown-referenced images into the output directory *preserving their subfolder*: `IMAGE_PATH` only affects images referenced via its own `\image` command, and `HTML_EXTRA_FILES` always flattens whatever it's given into the output root, not `img/`. So this repo's `img/` folder is copied into `doxygen-docs/html/img/` by hand (`cp img/*.png doxygen-docs/html/img/`) rather than through any Doxyfile setting — that manual copy is what the generated `<img src="img/...">` tags actually resolve against.

Separately (confirmed by testing, not assumed): Doxygen **also** auto-copies every locally-referenced Markdown image flat into the output root on *every* regenerate — this isn't a one-time artifact, it recurs on each `doxygen Doxyfile` run regardless of `IMAGE_PATH`/`HTML_EXTRA_FILES`. Those flat copies (`doxygen-docs/html/doxygen-preview.png` etc.) are unused — nothing references them, since the actual `<img src>` in the generated pages points at `img/...` — so delete them after each regenerate; they're harmless clutter, not a broken build. If new images are ever added to `img/` for the README, both steps apply: copy into `doxygen-docs/html/img/` by hand, then delete Doxygen's own flat auto-copy of the same file from the output root.
